import os,time,sys,pickle,nltk,statistics,re,os,time,sys,pickle,nltk,statistics,random
import pandas as pd
import numpy as np
...
#print("\nLength of Bigramized lines = {}".format(len(bigram_lines)))
...
#print("\nline {} in bigram form {}".format(lines[0],bigram_lines[0])) # [('6939', '4826'), ('4826', '38803'), ('38803', '56203')] 

#[print(bigram_lookup[i]) for i in test_path]

#print("sum of bigram counts:{0} and total bigrams:{1}".format(sum(bigram_values),len(bigram_values))) # 926785, 85056
...
class LanguageModel:
	...
	def bigram(self,sentence):
		# returns bigrams in each sentence
		if len(sentence.split(' ')) > 1:
			return(list(nltk.bigrams(sentence.split(' '))))
		elif len(sentence.split(' ')) == 1:
			return(list( (sentence.split(' ')[0],str(random.randint(5000,6000))) ))
	...
	def fit(self, corpus):
		...
		self.counter_ = Counter(word 
								 ...
								 for word in self.bigram(sentence))
...
for sent in lines:
	arr.append(lm.perplexity(sent))
print(lines[arr.index(min(arr))],lm.perplexity(lines[arr.index(min(arr))]))
